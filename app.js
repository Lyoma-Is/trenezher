...
// —— Сжатие картинок перед сохранением ——
// Картинки хранятся как base64 прямо в документе Firebase, поэтому крупные
// исходники сильно замедляют загрузку данных. Пережимаем в JPEG с разумным
// максимальным размером стороны — это на порядок уменьшает вес документа.
function compressImageDataUrl(rawDataUrl, opts) {
  opts = opts || {};
  const maxDim = opts.maxDim || 1600;
  const quality = opts.quality || 0.82;
  return new Promise(function(resolve) {
    if (!rawDataUrl || /^data:image\/gif/i.test(rawDataUrl)) {
      // GIF не трогаем — canvas.toDataURL убьёт анимацию
      resolve(rawDataUrl);
      return;
    }
    try {
      ...
      img.onload = function() {
        ...
        if (!w || !h) { resolve(rawDataUrl); return; }
        ...
        try {
          ...
          // используем сжатый вариант, только если он реально меньше исходного
          resolve(out && out.length < rawDataUrl.length ? out : rawDataUrl);
        } catch (e) {
          resolve(rawDataUrl);
        }
      };
...
function onceWithTimeout(ref, ms) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    var timer = setTimeout(function() {
      if (settled) return;
      settled = true;
      reject(new Error('Таймаут ' + Math.round(ms / 1000) + 'с'));
    }, ms);
    try {
      ref.once('value', function(snap) {
        ...
        settled = true;
        clearTimeout(timer);
        reject(err || new Error('Ошибка Firebase'));
      });
    } catch (e) {
      if (settled) return;
      settled = true;
...
/** Ждём готовности Firebase (SDK + init) — кроссбраузерно */
  ...
  return new Promise(function(resolve, reject) {
    ...
    (function tick() {
      ...
      if (Date.now() - start > maxMs) {
        reject(new Error('Firebase не готов'));
        return;
      }
      if (!window.firebase && typeof bootFirebase === 'function') {
        try { bootFirebase(); } catch (e) {}
      } else if (window.firebase && !firebaseReady) {
  ...
  return new Promise(function(resolve, reject) {
    try {
      ...
      if (!user || user.uid !== ADMIN_UID) {
        setSyncStatus('err', 'Нет прав записи');
        resolve(false);
        return;
      }
...
function pullFromCloud() {
  ...
  window._pullInFlight = Promise.all([
    ...
    cloudSynced = true;
    setSyncStatus(countContent(cloudCache) ? 'ok' : 'err', (e && e.message) || 'Ошибка');
    return false;
  });
 ...
 * конкретный уровень (first/highest) подгружается лениво через pullCategory,
...
function pullMetaFromCloud() {
  ...
  return Promise.all([
    ...
    if (!cloudCache) cloudCache = readLocalStore();
    cloudSynced = true;
    setSyncStatus(countContent(cloudCache) ? 'ok' : 'err', (e && e.message) || 'Нет связи');
    return false;
  });
}
...
function pullCategory(levelKey, parts) {
  ...
  return Promise.all(jobs).then(function() {
    ...
    setSyncStatus('err', (e && e.message) || 'Ошибка загрузки');
    return cloudCache[levelKey];
  });
}
...
function ensureCategoryLoaded(level, parts, force) {
  ...
  parts = parts || ['general', 'sectors'];
  if (!cloudCache) cloudCache = readLocalStore() || emptyStore();
  var side = cloudCache[lk] || { general: [], sectors: [] };

  var needG = parts.indexOf('general') >= 0;
  var needS = parts.indexOf('sectors') >= 0;
...
function onBlockImage(input) {
  ...
  reader.onload = () => {
    const raw = sanitizeImageSrc(reader.result);
    if (!raw) {
      appAlert('Некорректный файл изображения');
      return;
    }
    compressImageDataUrl(raw).then(function(src) {
...
function fileToDataUrl(file) {
  return new Promise(function(resolve, reject) {
    ...
    reader.onload = function() {
      compressImageDataUrl(reader.result).then(resolve).catch(function() { resolve(reader.result); });
    };
...
function initFirebase() {
  try {
    ...
    updateCustomCounts();
    applyHomeSettings();
    // загрузка данных — из bootFirebase (единая точка для всех браузеров)
    return true;
  } catch (e) {
...
document.addEventListener('DOMContentLoaded', function() {
  // 1) Мгновенный UI из localStorage — Chrome / Firefox / Safari / Edge / Яндекс
  try {
    cloudCache = readLocalStore();
  } catch (e) {
  ...
  if (cloudCache && cloudCache.updatedAt) {
    categoryFreshness.first = cloudCache.updatedAt;
    categoryFreshness.highest = cloudCache.updatedAt;
  }
  applyAdminUI();
  updateCustomCounts();
  applyHomeSettings();
  setSyncStatus(
    countContent(cloudCache) ? 'ok' : 'syncing',
    countContent(cloudCache) ? 'Из кэша' : 'Загрузка…'
  );
  if (!restoreQuizProgress()) {
    showPage('home-page');
  }
  // 2) Firebase + синхронизация (единый boot для всех браузеров)
  ...
  document.addEventListener('visibilitychange', function() {
    if (document.visibilityState === 'visible' && firebaseReady) {
      const prevUpdatedAt = lastCloudUpdatedAt;
      pullMetaFromCloud().then(function() {
        if (lastCloudUpdatedAt !== prevUpdatedAt) {
